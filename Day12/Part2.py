...
def build_path(currentPlace, currentPath, built_paths, has_visited_twice):
    if currentPlace.islower():
        # avoiding to repeat small caves
        if currentPlace == "start" and currentPath.count(currentPlace) > 0:
            return
        if currentPlace == "end" and currentPath.count(currentPlace) > 0:
            return
        if currentPath.count(currentPlace) > 0 and has_visited_twice:
            return
        elif currentPath.count(currentPlace) > 0:
    ...
    if this_place_paths != None:
        ...
        for place in this_place_paths:
            build_path(place, currentPath[:], built_paths, has_visited_twice)
...
for path in start:
    ...
    build_path(path, new_path[:], built_paths, False)
