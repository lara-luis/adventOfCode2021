...
for i,line in enumerate(cost_matrix):
    for x in range(1,extension_factor):
        cost_matrix[i].extend(this_line[:])
...
for i, line in enumerate(cost_matrix):
    for j, val in enumerate(line):
        if j + width < width*extension_factor:
            if i >= height and j < width:
                cost_matrix[i][j] = cost_matrix[i-height][j]+1 
                if cost_matrix[i][j] == 10:
                    cost_matrix[i][j] = 1

            cost_matrix[i][j+width] = cost_matrix[i][j] + 1
            if cost_matrix[i][j+width] == 10:
                cost_matrix[i][j+width] = 1

for i,line in enumerate(cost_matrix):
    for j, val in enumerate(line):
        gr.add_node((i,j))

        if i + 1 < width*extension_factor:
            gr.add_node((i+1,j))
            gr.add_edge((i+1,j), (i,j), weight=cost_matrix[i][j])
            gr.add_edge((i,j), (i+1,j), weight=cost_matrix[i+1][j])

        if j + 1 < height*extension_factor:
            gr.add_node((i,j+1))
            gr.add_edge((i,j+1), (i,j), weight=cost_matrix[i][j])
            gr.add_edge((i,j), (i,j+1), weight=cost_matrix[i][j+1])

min_path = nx.dijkstra_path(gr, source=(0,0), target=(width*extension_factor-1,height*extension_factor-1))
